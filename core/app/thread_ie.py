...
from core.utils.hutils import MyHTMLParser
...
class ThreadIE(threading.Thread):
    def __init__(self, aid=-1, auf='', alist=None, group=None, target=None, name=None, args=(), kwargs=None, verbose=None):
        super().__init__()
        self.CONTRIB_NOT_QUALIFIED: Final = 'Não habilitado como Contribuinte na UF'
        self.CONTRIB_QUALIFIED: Final = 'Habilitado como contribuinte na UF'
        self.CONTRIB_LOGICAL_EXCLUSION: Final = 'Exclusão lógica do Contribuinte na UF'
        if alist is None:
            alist = []
        self.id = aid
        self.uf = auf
    ...
    def parserIE(self, responseHTML, outlog, logFilename):
        ...
        if parser.data:
            ...
            parser.data = [sub.replace(self.CONTRIB_NOT_QUALIFIED, '0').replace(self.CONTRIB_QUALIFIED, '1').replace(self.CONTRIB_LOGICAL_EXCLUSION, '2') for sub in parser.data]
            vOut = ''
