                ...
                if request.exist:
                    ...
                    while self.__list_threads:
                        ...
                        print(
                            ...
                            f'{Fore.GREEN} | Found/NotFound/Total:{str(self.__count_cnpj_ie["found"]).zfill(qZero)}/{str(self.__count_cnpj_ie["n_found"]).zfill(qZero)}/{str(self.__count_cnpj_ie["found"] + self.__count_cnpj_ie["n_found"]).zfill(qZero)}',
                            end='',
                            flush=False)
    ...
    def __prepare_thread(self):
        try:
            for i in range(self.__num_threads):
                ...
                time.sleep(0.5)
                t = ThreadIE(aid=i, auf=self.__uf, alist=vList, aid_reg_bd=self.__request_id)
                ...
                while f.tell() < fsize:
                    ...
                    for _index, ss in enumerate(s):
                        if _index == len(s) - 1:
                            ...
                            values = values + f'"{ss}",'
                    records = records + (insert + values)
        if self.__monitor.execute_script(records):
            print(f'{Fore.GREEN}[*] All data successfully saved in the database')
